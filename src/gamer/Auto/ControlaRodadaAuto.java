...
import gamer.ui.utils.Deck;
import gamer.ui.utils.GameState;
...
public class ControlaRodadaAuto {
	int contadorMaos = 0;
	ControlaPartidaAuto controlaPartidaAuto = ControlaPartidaAuto.getInstacia();
	CbrModular trucoCbr_Agente1 = controlaPartidaAuto.getCBR(1);
	CbrModular trucoCbr_Agente2 = controlaPartidaAuto.getCBR(2);
	public List<String> historicoTentos;
	EstadoJogoModelo estadoJogo;
	...
	public ControlaRodadaAuto() {
		...
		utilCarta = false;
		hasDeception = false;
		envidoJustifications = "00000";
		trucoJustifications = "00000";
		deck = new Deck();
		deck.createDeck();
	...
	public void setaIndexacoesAgentes() {
		...
		//System.out.println("indexaÃ§Ãµes setadas: ");
	...
	Integer GanhadorPrimeira = null;
	Integer GanhadorSegunda = null;
	Integer GanhadorTerceira = null;
	...
	private void darCartas() {
		contadorMaos ++;
		//System.out.println("nova mÃ£o: "+contadorMaos);
		LimpaPontos();
...
//		System.out.println(listaCartasRecebidasAgente1);
//		System.out.println(listaCartasRecebidasAgente2);
		...
		//System.out.println("\nRECEBIDO2");
		//mostra_cartas(listaCartasRecebidasAgente2);
	...
	private static void mostra_cartas(List<CartasModelo> lista) {
		for (CartasModelo c : lista) {
			System.out.print("\t\t" + c.getCarta() + "- Imp = " + c.getValorImportancia() + "- Envido="
					+ c.getValorQueContaParaOenvido() + "-Id=" + c.getId());
		}
	}
	...
	public int quemJogaAgora() {
		...
		int quemJoga = 0;
//		System.out.println("sizes  " +listaCartasJogadasAgente1.size()+"  - "+listaCartasJogadasAgente2.size());
...
//		System.out.println("\t VErificando quem ganhou primeira mao  "+ listaCartasJogadasAgente1.size() + " - "+ listaCartasJogadasAgente2.size());
		int valorImportanciaPrimeiraCartaAgente1 = 0;
		int valorImportanciaPrimeiraCartaAgente2 = 0;
		if (listaCartasJogadasAgente1.size() >= 1) {
//			System.out.println("Pegando carta do 1");
...
//			System.out.println("Pegando carta do 2");
			valorImportanciaPrimeiraCartaAgente2 = listaCartasJogadasAgente2.get(0).getValorImportancia();
		}
//		System.out.println("\tcartas orig " +valorImportanciaPrimeiraCartaAgente1 + "     " + valorImportanciaPrimeiraCartaAgente2);
...
//		System.out.println("\tprimeira mao  "  + quemGanhou);
		if (listaCartasJogadasAgente1.size() == 1 && listaCartasJogadasAgente2.size() == 1) {
			GanhadorPrimeira = quemGanhou;
			//System.out.println("DEfine quem ganhou primeira mao" + quemGanhou);
		}
		return quemGanhou;
	}
...
//		System.out.println("\t VErificando quem ganhou segunda mao  "+ listaCartasJogadasAgente1.size() + " - "+ listaCartasJogadasAgente2.size());
...
//		System.out.println("\tcartas orig " + valorImportanciaSegundaCartaJogadaAgente1 + "     " + valorImportanciaSegundaCartaJogadaAgente2);
...
//			System.out.println("DEfine quem ganhou segunda mao" + quemGanhou);
		}
		//System.out.println("\tsegunda mao  " + quemGanhou);
		return quemGanhou;
	}
	...
	private int QuemGanhouOsPontos() {
		if (calcularPontosEnvidoAgente(1) > calcularPontosEnvidoAgente(2))
			return 1;
		else if (calcularPontosEnvidoAgente(2) > calcularPontosEnvidoAgente(1))
			return 2;
...
//			return verificaInformacoesJogada("ReTruco").getEmQualRodada();
	...
	public String ChamarPonto(int AgenteId) {
		...
		if (AgenteId == 1) {
			...
			if (cartasJogadasMesa.size() == 0 || (cartasJogadasMesa.size() == 1 && listaCartasJogadasAgente1.size() == 1)) {
				probGanhar = getProbabilidadeGanhar(controlaPartidaAuto.getQuemEhMao() == 1 ? 1 : 0, "ENVIDO");
				System.out.println("[PROBABILIDADE_ENVIDO_MAO] --> " + probGanhar);
			} else if (listaCartasJogadasAgente2.size() == 1 && listaCartasJogadasAgente1.size() == 0) {
				probGanhar = (1 - deck.getProbabilidadeMelhorEnvido(controlaPartidaAuto.getQuemEhMao() == 1 ? 1 : 0,
						calcularPontosEnvidoAgente(1), listaCartasJogadasAgente2.get(0).getCarta()));
				System.out.println("[PROBABILIDADE_ENVIDO_PE] --> " + probGanhar);
			}
		...
		if (AgenteId == 1) {
			...
			if (trucoCbr_Agente1.isActiveLearning() && !pontoQueSeraChamado.equals("Flor") &&
				...
				if (!pontoQueSeraChamado.equals("")) {
					...
					//if (trucoCbr_Agente1.getValidaCriterioDeveAprender().consultarEspecialista(probGanhar, 1)) {
					if (trucoCbr_Agente1.deveChamarTelaAtivoEmCadaAcaoIndividual("envido", isOportunidadeBlefe,
							contadorMao, atualizarConsultaEnvido(1,1), getDescriptionAtibutosBlefe())){
...
//		System.out.println("\nCHAMAR RODADA Agente " + AgenteId  );
		double probGanhar = 0.0;
		boolean isOportunidadeBlefe = false;
		int Oponente;
		if (AgenteId == 1) {
			Oponente = 2;
			probGanhar = getProbabilidadeGanhar(controlaPartidaAuto.getQuemEhMao() == 1 ? 1 : 0, "TRUCO");
		...
		if (AgenteId == 1) {
			...
			if (trucoCbr_Agente1.isActiveLearning()) {
				...
				if (!jogoQueSeraChamado.equals("")) {
					...
					//if (trucoCbr_Agente1.getValidaCriterioDeveAprender().consultarEspecialista(probGanhar, 2)) {

					if (trucoCbr_Agente1.deveChamarTelaAtivoEmCadaAcaoIndividual("truco", isOportunidadeBlefe,
							contadorMao, atualizarConsultaTruco(1,1), getDescriptionTrucoActiveLearning())){
					...
					if (trucoCbr_Agente1.deveChamarTelaAtivoEmCadaAcaoIndividual("truco", isOportunidadeBlefe,
							contadorMao, atualizarConsultaTruco(1, 1), getDescriptionAtibutosBlefe())){
						...
						responseActiveLearning = null;
						//System.out.println("RESPOSTA TELA: " + jogoQueSeraChamado);

						/*if (!jogoQueSeraChamado.equals("")) {

							jogada = getJogadaRealizada("TRUCO", jogoQueSeraChamado);

							if (isBluff2(controlaPartidaAuto.getQuemEhMao() == 1 ? 1 : 0, "TRUCO", jogada)) {
								hasDeception = true;
								utilEnvido = true;
							}
						} else {
						...
						//System.out.println("Active Learning Setado!!!!");
					}
				}
			}
		}
...
//		System.out.println("Verificiando se Agente "+ AgenteId + "  Aceita  Jogo " + jogadaParaSerAceita );
	...
	private boolean consultaChamarValeQuatro(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.chamarValeQuatro(atualizarConsultaTruco(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.chamarValeQuatro(atualizarConsultaTruco(2, 1), contadorMao);
	}

	private boolean consultaChamarRetruco(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.chamarReTruco(atualizarConsultaTruco(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.chamarReTruco(atualizarConsultaTruco(2, 1), contadorMao);
	}

	private boolean consultaChamarTruco(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.chamarTruco(atualizarConsultaTruco(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.chamarTruco(atualizarConsultaTruco(2, 1), contadorMao);
	}

	private boolean consultaChamarEnvido(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.chamarEnvido(atualizarConsultaEnvido(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.chamarEnvido(atualizarConsultaEnvido(2, 1), contadorMao);
	}

	private boolean consultaChamarRealEnvido(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.chamarRealEnvido(atualizarConsultaRealEnvido(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.chamarRealEnvido(atualizarConsultaRealEnvido(2, 1), contadorMao);
	}

	private boolean consultaChamarFaltaEnvido(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.chamarFaltaEnvido(atualizarConsultaFaltaEnvido(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.chamarFaltaEnvido(atualizarConsultaFaltaEnvido(2, 1), contadorMao);
	}
	...
	private boolean consultaAceitarValeQuatro(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.aceitarValeQuatro(atualizarConsultaTruco(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.aceitarValeQuatro(atualizarConsultaTruco(2, 1), contadorMao);
	}

	private boolean consultaAceitarRetruco(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.aceitarReTruco(atualizarConsultaTruco(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.aceitarReTruco(atualizarConsultaTruco(2, 1), contadorMao);
	}

	private boolean consultaAceitarTruco(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.aceitarTruco(atualizarConsultaTruco(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.aceitarTruco(atualizarConsultaTruco(2, 1), contadorMao);
	}

	private boolean consultaAceitarEnvido(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.aceitarEnvido(atualizarConsultaEnvido(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.aceitarEnvido(atualizarConsultaEnvido(2, 1), contadorMao);
	}

	private boolean consultaAceitarRealEnvido(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.aceitarRealEnvido(atualizarConsultaRealEnvido(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.aceitarRealEnvido(atualizarConsultaRealEnvido(2, 1), contadorMao);
	}

	private boolean consultaAceitarFaltaEnvido(int AgenteId) {
		if (AgenteId == 1)
			return trucoCbr_Agente1.aceitarFaltaEnvido(atualizarConsultaFaltaEnvido(1, 1), contadorMao);
		else
			return trucoCbr_Agente2.aceitarFaltaEnvido(atualizarConsultaFaltaEnvido(2, 1), contadorMao);
	}
	...
	public String jogarAgente(int quemJoga, int mao) {
		...
		if (quemJoga == 1) {
			...
			if (trucoCbr_Agente1.isActiveLearning()) {
					...
					if (trucoCbr_Agente1.deveChamarTelaAtivoEmCadaAcaoIndividual("carta", isOportunidadeBlefe,
							contadorMao, atualizarConsultaCarta(1, 1), getDescriptionAtibutosBlefe())){
...
//		System.out.println(quemGanhouAPrimeiraMao() + "    " + quemGanhouASegundaMao());
	...
	public void atualizaScoutBlefesRealizadosAgente() {
		...
		//probEnvido = 1-deck.getProbabilidadeMelhorEnvido(isHand, pontosEnvido);
		...
		if (QuemFlor == 0) {
			...
			if (isHand == 1) {
				...
				probEnvido = 1 - deck.getProbabilidadeMelhorEnvido(isHand, pontosEnvido, listaCartasJogadasAgente2.get(0).getCarta());
		...
		if (listaCartasJogadasAgente2.size() == 1) {
			probMao =1-deck.getProbabilidadeMelhorMao(isHand, listaCartasRecebidasAgente1.get(0).getCarta(),
					listaCartasRecebidasAgente1.get(1).getCarta(), 	listaCartasRecebidasAgente1.get(2).getCarta(),
			...
			probMao = 1-deck.getProbabilidadeMelhorMao(isHand, listaCartasRecebidasAgente1.get(0).getCarta(),
					listaCartasRecebidasAgente1.get(1).getCarta(), 	listaCartasRecebidasAgente1.get(2).getCarta(),
					listaCartasJogadasAgente2.get(0).getCarta(), listaCartasJogadasAgente2.get(1).getCarta());
		} else {
			probMao = 1-deck.getProbabilidadeMelhorMao(isHand, listaCartasRecebidasAgente1.get(0).getCarta(),
					listaCartasRecebidasAgente1.get(1).getCarta(), 	listaCartasRecebidasAgente1.get(2).getCarta());
		}
		...
		if (primeiraCartaOpponent) {
			probMao = 1-deck.getProbabilidadeMelhorMao(isHand, listaCartasRecebidasAgente1.get(0).getCarta(),
					listaCartasRecebidasAgente1.get(1).getCarta(), 	listaCartasRecebidasAgente1.get(2).getCarta(), listaCartasJogadasAgente2.get(0).getCarta());
		...
		if (quemGanhouAPrimeiraMao() == 2 && terceiraCartaOpponent) {
			...
			for (CartasModelo carta : cartasNaMao) {
				if (carta.getId() <= listaCartasJogadasAgente2.get(1).getId()) {
					...
					probMao = 1-deck.getProbabilidadeMelhorMao(1, listaCartasRecebidasAgente1.get(0).getCarta(),
							listaCartasRecebidasAgente1.get(1).getCarta(), 	listaCartasRecebidasAgente1.get(2).getCarta(),
							listaCartasJogadasAgente2.get(0).getCarta(), listaCartasJogadasAgente2.get(1).getCarta());
				}
	...
	public void atualizaScoutBlefesPlotadosOpponente() {
		...
		if (listaCartasJogadasAgente2.size() == 3 && cartasJogadasMesa.size() > 4) {
			...
			pontosEnvido = calcularPontosEnvidoAgente(2);
			probEnvido = 1-deck.getProbabilidadeMelhorEnvido(isHand, pontosEnvido);

			if (listaCartasJogadasAgente1.size() == 1) {
				probMao = 1-deck.getProbabilidadeMelhorMao(isHand, listaCartasRecebidasAgente2.get(0).getCarta(),
						listaCartasRecebidasAgente2.get(1).getCarta(), 	listaCartasRecebidasAgente2.get(2).getCarta());
			} else if (listaCartasJogadasAgente1.size() == 2) {
				probMao = 1-deck.getProbabilidadeMelhorMao(isHand, listaCartasRecebidasAgente2.get(0).getCarta(),
						listaCartasRecebidasAgente2.get(1).getCarta(), 	listaCartasRecebidasAgente2.get(2).getCarta());
			} else if (listaCartasJogadasAgente1.size() == 3) {

				if (listaCartasRecebidasAgente2.get(2).getId() > listaCartasRecebidasAgente1.get(2).getId() ||
						(quemGanhouAPrimeiraMao() == 2 &&
								listaCartasRecebidasAgente2.get(2).getId() == listaCartasRecebidasAgente1.get(2).getId())) {
					probMao = 1.0;
				} else {
					probMao = 0.0;
				}
			...
			if (isHand == 0) {
				...
				if ((QuemPediuEnvido == 0 && QuemPediuRealEnvido == 0 && QuemPediuFaltaEnvido == 0) ||
						(QuemPediuEnvido == 1 || QuemPediuRealEnvido == 1 || QuemPediuFaltaEnvido == 1) && probEnvido > 0.85) {
...
/*
			...
			if ((isHand == 0 && contadorMao == 1) || (quemGanhouAPrimeiraMao() == 2 && contadorMao == 2) ||
					(quemGanhouASegundaMao() == 2 && contadorMao == 2) || (quemGanhouATerceiraMao() == 2 && contadorMao == 3)) {
...
*/
			...
			try {
				if (listaCartasJogadasAgente1.size() > 2 && listaCartasJogadasAgente2.size() > 2) {
					...
					System.out.println("CartasJogadasAgente: " + listaCartasJogadasAgente1.size());
					System.out.println("CartasJogadasOpp: " + listaCartasJogadasAgente2.size());
					System.out.println("Cartas jogadas na mesa: " + cartasJogadasMesa.size());

					if ((listaCartasJogadasAgente2.get(2).getId() < listaCartasJogadasAgente1.get(2).getId() &&
							(QuemTruco == 2 && cartasJogadasMesa.size() >= 2) || (QuemRetruco == 2 && cartasJogadasMesa.size() >= 2) || QuemValeQuatro == 2 && cartasJogadasMesa.size()>= 2) ) {
			...
			if ((quemGanhouAPrimeiraMao() == 1 && quemGanhouASegundaMao() == 1 &&
					listaCartasJogadasAgente2.get(1).getId() <= listaCartasJogadasAgente1.get(1).getId() &&
					(QuemTruco == 2 && cartasJogadasMesa.size()>= 2) || (QuemRetruco == 2 && cartasJogadasMesa.size()>= 2) || QuemValeQuatro == 2 && cartasJogadasMesa.size()>= 2) ) {
...
//						System.out.println("\t\t2 Ganhou ");
	...
	private void pontuaPontosNaoAceitos() {
		...
		/*System.out.println("======= Declined Points ======");
		System.out.println("Pontos Agente 1: " + controlaPartidaAuto.getPontosAgente1());
		System.out.println("Pontos Agente 2: " + controlaPartidaAuto.getPontosAgente2());
		System.out.println("--- lastDeclinedPoint: " + lastDeclinedPoint);
		System.out.println("--- lastCalledPoint: " + lastCalledPoint);
		System.out.println("--- lastPlayerCallPoints: " + lastPlayerCallPoints);*/
		...
		// verifica se a pontuaÃƒÂ§ÃƒÂ£o nÃƒÂ£o aceita foi envido e pontua quem chamou em 1
		// ponto
		/*if (listPontosNaoAceitos.get(listPontosNaoAceitos.size() - 1).getJogadaNaoAceita().equals("Envido")) {
			...
			if (listaPontosChamados.get(listaPontosChamados.size() - 1).getQuemChamou() == 1 && listaPontosChamados
				...
				controlaPartidaAuto.setPontosAgente1(controlaPartidaAuto.getPontosAgente1() + 1);
				tentosRodada.setEnvido(1, 1);
			} else if (listaPontosChamados.get(listaPontosChamados.size() - 1).getQuemChamou() == 2 && listaPontosChamados
					.get(listaPontosChamados.size() - 1).getJogadaChamada().equalsIgnoreCase("Envido")) {
				controlaPartidaAuto.setPontosAgente2(controlaPartidaAuto.getPontosAgente2() + 1);
				tentosRodada.setEnvido(2, 1);
			}
		} else*/
		...
		// verifica se foi chamado Envido e Real envido, pontua em 2 pontos quem chamou
	/*	if (!listaPontosAceitos.isEmpty() && !listPontosNaoAceitos.isEmpty()
				&& listPontosNaoAceitos.get(listPontosNaoAceitos.size() - 1).getJogadaNaoAceita().equals("FaltaEnvido")
				&& listaPontosAceitos.get(listaPontosAceitos.size() - 1).getJogadaAceita().equals("Envido")) {
			if (listaPontosChamados.get(listaPontosChamados.size() - 1).getQuemChamou() == 1 && listaPontosChamados
					.get(listaPontosChamados.size() - 1).getJogadaChamada().equalsIgnoreCase("FaltaEnvido")) {
				controlaPartidaAuto.setPontosAgente1(controlaPartidaAuto.getPontosAgente1() + 2);
				tentosRodada.setEnvido(1, 2);
			} else if (listaPontosChamados.get(listaPontosChamados.size() - 1).getQuemChamou() == 2 && listaPontosChamados
					.get(listaPontosChamados.size() - 1).getJogadaChamada().equalsIgnoreCase("RealEnvido")) {
		...
		}*/ /*else if (listPontosNaoAceitos.get(listPontosNaoAceitos.size() - 1).getJogadaNaoAceita().equals("FaltaEnvido")
				&& listaPontosAceitos.isEmpty()) {
			if (listaPontosChamados.get(listaPontosChamados.size() - 1).getQuemChamou() == 1 && listaPontosChamados
					.get(listaPontosChamados.size() - 1).getJogadaChamada().equalsIgnoreCase("FaltaEnvido")) {
				controlaPartidaAuto.setPontosAgente1(controlaPartidaAuto.getPontosAgente1() + 1);
	...
	/*public boolean isBluff2(int isHand, String typeJogada, String jogada) {

		boolean bluff = false;
		boolean poucosPontos = (1-deck.getProbabilidadeMelhorEnvido(isHand, calcularPontosEnvidoAgente(1))) < 0.5;
		boolean muitosPontos = (1-deck.getProbabilidadeMelhorEnvido(isHand, calcularPontosEnvidoAgente(1))) > 0.9;
		boolean maoForte = (1-getProbabilidadeMaoByEstadoJogo()) > 0.85;
		boolean maoFraca = (1-getProbabilidadeMaoByEstadoJogo()) < 0.26;

		switch (typeJogada) {
			case "ENVIDO":
				if (jogada.equals("NO_CALL") && muitosPontos) {
					bluff = true;
				} else if ((jogada.equals("ENVIDO") || jogada.equals("REAL_ENVIDO") || jogada.equals("FALTA_ENVIDO")) && poucosPontos) {
					bluff = true;
				} else if (jogada.startsWith("ACCEPT") && poucosPontos) {
					bluff = true;
				}
				break;
			case "TRUCO":
				if (jogada.equals("NO_CALL") && maoForte) {
					bluff = true;
				} else if ((jogada.equals("TRUCO") || jogada.equals("RETRUCO") || jogada.equals("VALE4")) && maoFraca) {
					bluff = true;
				} else if (jogada.startsWith("ACCEPT") && maoFraca) {
					bluff = true;
				}
				break;
			case "PLAY_CARD":
	...
	public String getEnvidoJustifications() {
		return envidoJustifications;
	}

	public void setEnvidoJustifications(String envidoJustifications) {
		this.envidoJustifications = envidoJustifications;
	}

	public String getTrucoJustifications() {
		return trucoJustifications;
	}

	public void setTrucoJustifications(String trucoJustifications) {
		this.trucoJustifications = trucoJustifications;
	}
